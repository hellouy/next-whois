...
- [ ] 1. 扩展数据模型与共享类型
  - [ ] 1.1 扩展 `expired_domain_leads` 表结构
...
- [ ] 2. 实现日期解析与阶段归类
  - [ ] 2.1 实现 `parseDropDate` 多格式解析
    - 支持 `YYYY-MM-DD`、`DD-MMM-YYYY`、`YYYY`，解析失败返回 null（对应 R2.1）

  - [ ] 2.2 实现 `classifyStage` 阶段归类
...
- [ ] 3. 实现多维价值评分扩展
  ...
  - [ ] 3.3 实现 `isDictionaryWord` 单词判定
    - 精确匹配词表，按词长分级给分（3-4 字母满分）（对应 R4.2）

  - [ ] 3.4 实现 `splitPinyin` 双拼判定
    - 动态规划将纯字母 SLD 切分为 2-4 个有效音节且无剩余，命中常用词额外加分（对应 R4.2）

  - [ ] 3.5 实现 `scoreDomainExtended` 七维评分
...
- [ ] 5. 实现采集器层
  ...
  - [ ] 5.2 实现 expireddomains 待删除视图采集器
    - 复用现有登录流程与表格解析，新增待删除视图抓取（对应 R1.1）

  - [ ] 5.3 实现 whoisds 每日列表采集器
