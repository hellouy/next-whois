...
async function markRetry(id: number, error: string, delayMinutes: number): Promise<void> {
  ...
  await dbQuery(
    `UPDATE email_queue
        SET attempts      = attempts + 1,
            ...
            next_retry_at = $3
      WHERE id = $1`,
    [id, error.slice(0, 500), nextAt.toISOString()],
  );
}
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (nextAttempt >= row.max_attempts) {
        ...
        await markRetry(row.id, errMsg, delay);
        result.retried++;
