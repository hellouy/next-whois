/**
 * Static RDAP server bootstrap derived from IANA dns.json bootstrap file.
 * Source: https://data.iana.org/rdap/dns.json
 * Generated: 2026-03-27
 *
 * Contains 1198 TLD→RDAP server mappings covering gTLDs + ccTLDs.
 * ccTLD entries here are superseded by CCTLD_RDAP_OVERRIDES in rdap_client.ts,
 * which contains hand-curated, more accurate endpoints for 168 ccTLDs.
 *
 * This eliminates the node-rdap IANA network round-trip for all known TLDs,
 * dramatically improving first-query latency for any TLD we have locally.
 */
const GTLD_RDAP_BOOTSTRAP: Record<string, string> = {
...
/**
 * Look up the RDAP server for a TLD from the local IANA bootstrap.
 * Returns undefined for TLDs not in our local map (node-rdap fallback used).
 */
